  ...
  <div>
    <v-dialog v-model="dialog" max-width="600">
      <template v-slot:activator="{ props: activatorProps }">
        <v-btn class="text-none font-weight-regular ml-0" prepend-icon="mdi-account-group" text="Registrar Personal"
          variant="tonal" size="x-large" rounded v-bind="activatorProps"></v-btn>
      </template>

      <v-card>
        <v-card-title class="d-flex justify-space-between align-center">
          <div class="text-h6 text-medium-emphasis">Registro de Personal</div>
            ...
            <v-stepper-window class="pa-0 mx-0">
              ...
              <v-stepper-window-item :value="3">
                <v-form class="pa-0">
                  <v-row dense>
                    <v-col cols="12" sm="6">
                      <v-text-field v-model="conductor.nro_licencia" label="N° Licencia" required />
                    </v-col>
                    <v-col cols="12" sm="6">
                      <v-text-field v-model="conductor.categoria" label="Categoria" required />
                    </v-col>
                    <v-col cols="12" sm="6">
                      <v-text-field v-model="conductor.expedicion" type="date" label="Fecha de expedicion" required />
                    </v-col>
                    <v-col cols="12" sm="6">
                      <v-text-field v-model="conductor.expiracion" type="date" label="Fecha de expiracion" required />
                    </v-col>
                  </v-row>
                </v-form>
              </v-stepper-window-item>
...
const guardar = () => {
  ...
  console.log('Conductor:', conductor.value)
  dialog.value = false
